...
import { _decorator, Component, Node, Button, Label, tween, UITransform, Color, Vec3 } from 'cc';
...
export class MarqueeUI extends Component {
    ...
    private _createUI(): void {
        ...
        // 标题
        const titleLabel = this._createLabelOn('Title', '🏆 跑马灯', 28, new Color(255, 215, 0), new Vec3(0, 110, 0), this._panel);

        // 提示文字
        this._progressLabel = this._createLabelOn('Progress', '看广告点亮跑马灯，3格领钥匙！', 18, new Color(200, 200, 200), new Vec3(0, 70, 0), this._panel);
    ...
    private _saveProgress(): void {
        try {
            if (typeof wx !== 'undefined' && wx.setStorageSync) {
                wx.setStorageSync('marquee_progress', JSON.stringify(this._lights));
            } else {
                localStorage.setItem('marquee_progress', JSON.stringify(this._lights));
            }
        } catch (err) {
    ...
    private _loadProgress(): void {
        try {
            ...
            if (typeof wx !== 'undefined' && wx.getStorageSync) {
                raw = wx.getStorageSync('marquee_progress');
            } else {
                raw = localStorage.getItem('marquee_progress');
            }
            if (raw) {
