  ...
  return {
    ...
    execute: function () {
      ...
      _export("EcsEntityBridge", EcsEntityBridge = (_dec = ccclass('EcsEntityBridge'), _dec(_class = (_class2 = (_class3 = class EcsEntityBridge extends Component {
        ...
        setSpawn(x, y, team, isBoss = false) {
          this._startX = x;
          this._startY = y;
          this._team = team;
          this._isBoss = isBoss;
        ...
        onLoad() {
          ...
          const opts = {
            ...
            isBoss: this._isBoss,
            dispatch: cmd => {
              const cs = ctx.getOptional(_crd && ICombatSystem === void 0 ? (_reportPossibleCrUseOfICombatSystem({
                error: Error()
              }), ICombatSystem) : ICombatSystem);
              cs == null || cs.dispatch(cmd);
            }
          };
          const built = (_crd && EcsEntityFactory === void 0 ? (_reportPossibleCrUseOfEcsEntityFactory({
            error: Error()
          }), EcsEntityFactory) : EcsEntityFactory).build(ctx, opts);
          const em = ctx.getOptional(_crd && IEntityManager === void 0 ? (_reportPossibleCrUseOfIEntityManager({
            error: Error()
