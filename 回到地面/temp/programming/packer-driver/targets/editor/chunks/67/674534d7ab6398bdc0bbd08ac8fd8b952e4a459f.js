    ...
    execute: function () {
      ...
      _export("PlayerController", PlayerController = (_dec = ccclass('PlayerController'), _dec(_class = (_class2 = class PlayerController extends Component {
        ...
        _calcDamage(rawDamage, stats) {
          const dmgRng = (_crd && RunRng === void 0 ? (_reportPossibleCrUseOfRunRng({
            error: Error()
          }), RunRng) : RunRng).instance.fork('player:damageCalc');
          const d6Roll = dmgRng.d6();
          const reduced = Math.floor(stats.def * (_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
            error: Error()
          }), GameConfig) : GameConfig).DAMAGE_FORMULA_DEF_FACTOR);
          const afterDef = Math.max((_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
            error: Error()
        ...
        heal(amount) {
          ...
          const effectiveAmount = pdm.selectedTalent === 'iron_stomach' ? Math.floor(amount * 1.3) : amount;
