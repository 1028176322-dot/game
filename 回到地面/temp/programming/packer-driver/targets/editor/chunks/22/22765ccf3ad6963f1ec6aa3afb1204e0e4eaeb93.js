  ...
  return {
    ...
    execute: function () {
      ...
      _export("Logger", Logger = class Logger {
        constructor(isDev = true, channelLevels, sink) {
          this.sink = void 0;
          this.levels = new Map();
          ...
          for (const name of DEFAULT_CHANNELS) {
            var _channelLevels$name;

            this.levels.set(name, (_channelLevels$name = channelLevels == null ? void 0 : channelLevels[name]) != null ? _channelLevels$name : defaultLevel);
        ...
        makeChannel(name) {
          const emit = (level, msg, meta) => {
            ...
            const threshold = (_this$levels$get = this.levels.get(name)) != null ? _this$levels$get : LogLevel.Debug;
