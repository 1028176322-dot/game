  ...
  function buildReactions() {
    ...
    r.set(reactionKey((_crd && ElementType === void 0 ? (_reportPossibleCrUseOfElementType({
      ...
      handler: (sys, monster, pos) => {
        ...
        // 额外造成一次 100% ATK 伤害
        const extraDmg = Math.max((_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
          error: Error()
    ...
    r.set(reactionKey((_crd && ElementType === void 0 ? (_reportPossibleCrUseOfElementType({
      ...
      handler: (sys, monster, pos) => {
        ...
        const dmg = Math.max((_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
          error: Error()
    ...
    r.set(reactionKey((_crd && ElementType === void 0 ? (_reportPossibleCrUseOfElementType({
      ...
      handler: (sys, monster, pos) => {
        // 给相邻怪物也随机附上 Poison
        const neighbors = sys._getNearestMonsters(pos, (_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
          error: Error()
    ...
    r.set(reactionKey((_crd && ElementType === void 0 ? (_reportPossibleCrUseOfElementType({
      ...
      handler: (sys, monster, _pos) => {
        ...
        const baseDmg = Math.max((_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
          error: Error()
    ...
    const purgeElements = [(_crd && ElementType === void 0 ? (_reportPossibleCrUseOfElementType({
      error: Error()
    }), ElementType) : ElementType).Fire, (_crd && ElementType === void 0 ? (_reportPossibleCrUseOfElementType({
      error: Error()
    }), ElementType) : ElementType).Frost, (_crd && ElementType === void 0 ? (_reportPossibleCrUseOfElementType({
      ...
      _export("ElementSystem", ElementSystem = (_dec = ccclass('ElementSystem'), _dec(_class = (_class2 = class ElementSystem extends Component {
        ...
        _tryTriggerReaction(monster, newElement, pos, chainDepth) {
          ...
          for (const status of existing) {
            ...
            const reaction = (_ElementSystem$_REACT = ElementSystem._REACTIONS) == null ? void 0 : _ElementSystem$_REACT.get(key);
        ...
        _attachElement(monster, element, duration, stacks, tickTimer, tickDamage, tickInterval, tickStartDelay) {
          ...
          const list = this._monsterStatus.get(monster); // 同元素叠加
        ...
        _applyDot(monster, element, tickDamage, tickInterval, totalDuration, rampUp = false) {
          ...
          for (const [monster, statuses] of this._monsterStatus.entries()) {
            ...
            for (let i = statuses.length - 1; i >= 0; i--) {
              ...
              if (s.remainingTime <= 0) {
                (_crd && eventBus === void 0 ? (_reportPossibleCrUseOfeventBus({
                  error: Error()
                }), eventBus) : eventBus).emit('element:removed', monster, s.element);
                statuses.splice(i, 1);
              ...
              if (s.tickDamage > 0 && s.tickInterval > 0) {
                ...
                if (s.tickTimer <= 0) {
                  ...
                  if (!monster.isDead) {
                    ...
                    (_crd && eventBus === void 0 ? (_reportPossibleCrUseOfeventBus({
                      error: Error()
                    }), eventBus) : eventBus).emit('element:dot_tick', monster, s.element, dot);
