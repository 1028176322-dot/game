  ...
  return {
    ...
    execute: function () {
      ...
      _export("FSMAIController", FSMAIController = class FSMAIController {
        ...
        decide(self, p) {
          const dist = p.targetAlive ? manhattan(self.gridX, self.gridY, p.targetX, p.targetY) : 999; // Transition (record lookup, no switch on id).
      ...
      _export("GOAPController", GOAPController = class GOAPController {
        ...
        _plan(start, actions) {
          ...
          const seen = new Set([worldKey(start)]);
