  ...
  return {
    ...
    execute: function () {
      ...
      _export("MutationManager", MutationManager = class MutationManager {
        ...
        /** 为新楼层生成变异 */
        generateMutation(floorNumber, playerDiedRecently = false) {
          ...
          const mutationRng = (_crd && RunRng === void 0 ? (_reportPossibleCrUseOfRunRng({
            error: Error()
        ...
        _pickMutation(excludeIds) {
          ...
          const candidates = ALL_MUTATIONS.filter(m => !excludeIds.includes(m.id)); // 检查互斥
        ...
        _applyMutations() {
          for (const mutation of this._activeMutations) {
            const eff = mutation.effect; // 发送参数覆盖事件
