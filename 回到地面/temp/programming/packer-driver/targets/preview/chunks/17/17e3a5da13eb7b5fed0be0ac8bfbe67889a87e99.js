  ...
  return {
    ...
    execute: function () {
      ...
      _export("PoolManager", PoolManager = class PoolManager {
        ...
        registerPool(name, prefab, preCreate, maxSize) {
          if (preCreate === void 0) {
            preCreate = 5;
          }
        ...
        get(name, parent) {
          var entry = this._pools.get(name);
          ...
          var node = null; // 找池中空闲对象

          for (var i = entry.pool.length - 1; i >= 0; i--) {
        ...
        clear() {
          for (var [, entry] of this._pools) {
            for (var node of entry.pool) {
              node.destroy();
            }
          }
