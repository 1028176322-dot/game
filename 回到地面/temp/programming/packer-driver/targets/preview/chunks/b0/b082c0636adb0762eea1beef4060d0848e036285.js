    ...
    execute: function () {
      ...
      _export("EventUI", EventUI = (_dec = ccclass('EventUI'), _dec2 = property(Prefab), _dec(_class = (_class2 = class EventUI extends Component {
        ...
        _createUI() {
          ...
          var maskBtn = mask.addComponent(Button); // 遮罩点击无效（不关闭面板）
          ...
          var aPanel = new Node('OptionA_Container');
          aPanel.setPosition(-180, -80, 0);
          var aTransform = aPanel.addComponent(UITransform);
          aTransform.setContentSize(260, 200);
          var aBtn = aPanel.addComponent(Button);
          aBtn.clickEvents = [{
            target: this.node,
            component: 'EventUI',
          ...
          var bPanel = new Node('OptionB_Container');
          bPanel.setPosition(180, -80, 0);
          var bTransform = bPanel.addComponent(UITransform);
          bTransform.setContentSize(260, 200);
          var bBtn = bPanel.addComponent(Button);
          bBtn.clickEvents = [{
            target: this.node,
            component: 'EventUI',
        ...
        _onEnterEventRoom(roomId) {
          ...
          var floor = gm ? gm.currentFloor : 1;
          var zoneId = gm ? gm.currentZone : 'forest';
        ...
        _resolveChoice(isA) {
          ...
          this._isActive = false;
          var option = isA ? this._currentEvent.optionA : this._currentEvent.optionB;

          if (this._eventSystem) {
            for (var consequence of option.consequences) {
        ...
        _updateTimerDisplay() {
          if (this._timerLabel) {
            var secs = Math.ceil(this._autoSelectTimer);
