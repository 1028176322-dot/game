    ...
    execute: function () {
      ...
      _export("AdService", AdService = class AdService {
        ...
        playRewardedAd(placement, callback) {
          ...
          var adUnitId = this._getAdUnitId(placement);
          ...
          try {
            var wrapperRewardedAd = this._adCache.get(placement);

            if (!wrapperRewardedAd) {
              var ad = this._adapter.createRewardedAd(adUnitId);
                ...
                show: () => {
                  return new Promise((resolve, reject) => {
                    ad.onClose(res => {
                      ...
                      var rewarded = (_res$isEnded = res == null ? void 0 : res.isEnded) != null ? _res$isEnded : false;
                      callback == null || callback({
                        success: true,
                        rewarded
                      });
        ...
        tryShowInterstitial() {
          ...
          try {
            var adUnitId = this._getAdUnitId((_crd && AdPlacement === void 0 ? (_reportPossibleCrUseOfAdPlacement({
              error: Error()
            }), AdPlacement) : AdPlacement).Interstitial);

            if (!adUnitId) return;

            var ad = this._adapter.createInterstitialAd(adUnitId);
        ...
        showBanner() {
          ...
          try {
            ...
            var bAd = this._adapter.createBannerAd(adUnitId, {
              left: 0,
              top: 0,
              width: 320
            });
        ...
        _isAdOnCD(placement) {
          var last = this._adCD.get(placement) || 0;
