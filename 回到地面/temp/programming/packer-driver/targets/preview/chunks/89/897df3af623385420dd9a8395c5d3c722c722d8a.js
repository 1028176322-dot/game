      ...
      _export("PlatformService", PlatformService = class PlatformService {
        ...
        init(options) {
          ...
          return _asyncToGenerator(function* () {
            var opt = options || {};
            _this._channel = opt.channel || '';
            _this._adapter = _this._createAdapter();
            console.log("[PlatformService] init: platform=" + _this._platform + ", channel=" + _this._channel + ", adapter=" + _this._adapter.platformId);
            yield _this._adapter.init(); // Migrate legacy storage keys

            _this._migrateLegacyKeys();
          })();
        } // ── Platform Detection ──
        ...
        _migrateLegacyKeys() {
          try {
            ...
            if (!storage) return;
            var platformUserId = storage.getItem('platform_user_id');
            if (platformUserId) return; // already migrated

            var wxOpenId = storage.getItem('wx_openid');
        ...
        logout() {
          ...
          return _asyncToGenerator(function* () {
            if (_this3._adapter) yield _this3._adapter.logout();
          })();
        }
        ...
        checkCompliance(userId) {
          ...
          return _asyncToGenerator(function* () {
            ...
            return _this4._adapter.checkCompliance(userId);
          })();
        }
