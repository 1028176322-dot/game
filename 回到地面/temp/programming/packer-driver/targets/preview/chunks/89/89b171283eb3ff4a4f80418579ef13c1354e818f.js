  ...
  return {
    ...
    execute: function () {
      ...
      _export("PlayerDataManager", PlayerDataManager = class PlayerDataManager {
        ...
        getCharacterName() {
          var explicitName = this._data.characterName;

          if (explicitName) {
            return explicitName;
          }

          var character = CHARACTER_LIST.find(c => c.id === this.selectedCharacter);
        ...
        unlockCharacter(charId) {
          ...
          var charDef = CHARACTER_LIST.find(c => c.id === charId);
        ...
        purchaseTalent(talentId) {
          var talentDef = TALENT_LIST.find(t => t.id === talentId);
        ...
        _load() {
          try {
            var saveService = (_crd && SaveService === void 0 ? (_reportPossibleCrUseOfSaveService({
              error: Error()
            }), SaveService) : SaveService).instance;
            var profile = saveService.loadProfile();
            var data = profileToSaveData(profile);
        ...
        _save() {
          try {
            var profile = saveDataToProfile(this._data);
