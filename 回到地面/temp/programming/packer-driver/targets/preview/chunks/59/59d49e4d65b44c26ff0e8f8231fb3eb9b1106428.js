    ...
    execute: function () {
      ...
      _export("CombatComponent", CombatComponent = class CombatComponent {
        ...
        process(sourceId) {
          ...
          var req = this._queue.shift(); // Check cooldown.


          var remaining = (_this$_cooldowns$get = this._cooldowns.get(req.skillId)) != null ? _this$_cooldowns$get : 0;
        ...
        update(dt) {
          ...
          for (var [id, remaining] of this._cooldowns) {
            var newRem = remaining - dt;
        ...
        static resolveAutoAttack(ctx, target, rollCrit) {
          if (rollCrit === void 0) {
            rollCrit = () => Math.random() < ctx.critChance;
          }
          ...
          var isCrit = rollCrit();
          var raw = isCrit ? Math.floor(ctx.atk * ctx.critMultiplier) : ctx.atk;
          var damage = Math.max(1, Math.floor(raw * ctx.damageMultiplier));
          var killed = target.applyDamage(damage, isCrit);
          var lifeStealHeal = damage > 0 && ctx.lifeSteal > 0 ? Math.max(1, Math.floor(damage * ctx.lifeSteal)) : 0;
          return {
            performed: true,
            damage,
            isCrit,
