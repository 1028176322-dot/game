System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2", "__unresolved_3"], function (_export, _context) {
  ...
  function rollRarity() {
    ...
    var total = RARITY_WEIGHTS.reduce((s, e) => s + e.weight, 0);
  ...
  function rollAffixes(rarity, pool) {
    ...
    var affix = (_crd && MathUtils === void 0 ? (_reportPossibleCrUseOfMathUtils({
      error: Error()
    }), MathUtils) : MathUtils).randomPick(pool);
    var rarityLevel = RARITY_WEIGHTS.findIndex(e => e.rarity === rarity);
  ...
  function generateEquipment(slot, forceRarity) {
    ...
    var actualSlot = slot != null ? slot : randomSlot();
    var rarity = forceRarity != null ? forceRarity : rollRarity();
    var baseStats = (_SLOT_BASE_STATS$actu = SLOT_BASE_STATS[actualSlot]) != null ? _SLOT_BASE_STATS$actu : {}; // 稀有度乘数: 白 1x, 蓝 1.5x, 金 2x, 橙 3x

    var rarityMult = [1, 1.5, 2, 3][RARITY_WEIGHTS.findIndex(e => e.rarity === rarity)];
    var scaledBase = {};

    for (var [key, val] of Object.entries(baseStats)) {
      scaledBase[key] = Math.floor(val * rarityMult);
    } // 是否套装饰品 (30% 概率)


    var _id = ++_equipIdCounter;

    var equipRng = (_crd && RunRng === void 0 ? (_reportPossibleCrUseOfRunRng({
      error: Error()
    }), RunRng) : RunRng).instance.fork("equip:generate:" + _id);
    var isSet = equipRng.chance(0.3);
    var setBonusId = isSet ? (_crd && MathUtils === void 0 ? (_reportPossibleCrUseOfMathUtils({
      error: Error()
    }), MathUtils) : MathUtils).randomPick(SET_BONUS_IDS) : null; // 生成词缀

    var prefix = rarity === Rarity.Common ? null : rollAffixes(rarity, PREFIXES);
    var suffix = rarity === Rarity.Common || rarity === Rarity.Uncommon ? null : rollAffixes(rarity, SUFFIXES);
    var id = "equip_" + _id + "_" + (_crd && RunRng === void 0 ? (_reportPossibleCrUseOfRunRng({
      error: Error()
    }), RunRng) : RunRng).instance.seed;

    var name = _buildEquipName(actualSlot, prefix, suffix, rarity);
  ...
  function _buildEquipName(slot, prefix, suffix, rarity) {
    ...
    var slotName = (_SLOT_NAMES$slot = SLOT_NAMES[slot]) != null ? _SLOT_NAMES$slot : '装备';
    var rarityTag = RARITY_PREFIX[rarity] ? "[" + RARITY_PREFIX[rarity] + "]" : '';
    var pre = prefix ? prefix.name : '';
    var suf = suffix ? suffix.name : '';
    var parts = [rarityTag, pre, slotName, suf].filter(Boolean);
    return parts.join('');
  }
    ...
    execute: function () {
      ...
      _export("EquipmentSystem", EquipmentSystem = (_dec = ccclass('EquipmentSystem'), _dec(_class = class EquipmentSystem extends Component {
        ...
        equip(item, targetSlot) {
          ...
          var slot = targetSlot != null ? targetSlot : item.slot; // 检查槽位匹配
          ...
          var old = (_this$_equipped$get = this._equipped.get(slot)) != null ? _this$_equipped$get : null;
        ...
        unequip(slot) {
          ...
          var item = (_this$_equipped$get2 = this._equipped.get(slot)) != null ? _this$_equipped$get2 : null;
          if (!item) return null; // 尝试放入背包

          var idx = this._backpack.findIndex(e => e === null);
        ...
        swapBackpack(from, to) {
          var a = this._backpack[from];
          var b = this._backpack[to];
        ...
        _applyEquipStats() {
          ...
          for (var [slot, item] of this._equipped) {
            // 基础属性
            for (var [stat, val] of Object.entries(item.baseStats)) {
              if (val !== 0) {
        ...
        _recalcSetBonuses() {
          ...
          var setCount = new Map();

          for (var [, item] of this._equipped) {
            if (item.setBonusId) {
              var _setCount$get;
          ...
          for (var bonus of this._setBonuses) {
            var _setCount$get2;

            var count = (_setCount$get2 = setCount.get(bonus.id)) != null ? _setCount$get2 : 0;

            for (var effect of bonus.pieceEffects) {
              if (count >= effect.count) {
                var source = "set:" + bonus.id + ":" + effect.count;
        ...
        _clearSetBonuses() {
          for (var src of this._activeSetSources) {
        ...
        getActiveSetBonuses() {
          ...
          var _loop = function _loop() {
            ...
            var count = (_setCount$get4 = setCount.get(bonus.id)) != null ? _setCount$get4 : 0;

            if (count > 0) {
              var activeEffects = bonus.pieceEffects.filter(e => count >= e.count).map(e => e.description);
              result.push({
                id: bonus.id,
                name: bonus.name,
                count,
        ...
        generateDrops(roomType, count) {
          ...
          for (var i = 0; i < count * 2; i++) {
            var drop = this.generateDrop(roomType);
        ...
        getRarityCounts() {
          ...
          for (var item of this._backpack) {
            if (item) counts[item.rarity]++;
          }

          for (var [, _item] of this._equipped) {
            if (_item) counts[_item.rarity]++;
          }
        ...
        getTotalPowerLevel() {
          ...
          for (var [, item] of this._equipped) {
            ...
            for (var val of Object.values(item.baseStats)) {
              power += Math.abs(val);
            }
