    ...
    execute: function () {
      ...
      _export("EquipmentUI", EquipmentUI = (_dec = ccclass('EquipmentUI'), _dec2 = property(Prefab), _dec(_class = (_class2 = class EquipmentUI extends Component {
        ...
        _refreshAll() {
          ...
          var vm = this._buildVM();

          this._view.render(vm);
        }

        _buildVM() {
          var es = this._equipSystem;
          var slots = (_crd && EQUIPMENT_SLOTS === void 0 ? (_reportPossibleCrUseOfEQUIPMENT_SLOTS({
            error: Error()
          ...
          var bp = es.getBackpack();
          var backpack = bp.map((item, i) => ({
            index: i,
            item: this._toItemVM(item)
          }));
          var sets = es.getActiveSetBonuses();
          return {
            slots,
            backpack,
        ...
        _onSlotClick(slot) {
          ...
          var item = this._equipSystem.getEquipped(slot);
        ...
        _onBackpackClick(index) {
          ...
          var bp = this._equipSystem.getBackpack();
