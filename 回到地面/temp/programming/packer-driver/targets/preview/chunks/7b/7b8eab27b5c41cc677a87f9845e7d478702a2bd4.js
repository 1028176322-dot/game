System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  function deriveSeed(input, salt) {
    var hex = (_crd && computeConfigHash === void 0 ? (_reportPossibleCrUseOfcomputeConfigHash({
      error: Error()
  ...
  function buildSeedChain(userSeed, floorIndex, nodeId) {
    var runSeed = userSeed;
    var floorSeed = deriveSeed(runSeed, "f" + floorIndex);
    var routeSeed = deriveSeed(floorSeed, 'route'); // NodeRouteMapDefinition.seed

    var nodeSeed = deriveSeed(floorSeed, nodeId);
    var rewardSeed = deriveSeed(nodeSeed, 'reward');
    var encounterSeed = deriveSeed(nodeSeed, 'encounter'); // -> config.seed
