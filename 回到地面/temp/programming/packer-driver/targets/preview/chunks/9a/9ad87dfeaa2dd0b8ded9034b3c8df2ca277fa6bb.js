    ...
    execute: function () {
      ...
      _export("DungeonManager", DungeonManager = (_dec = ccclass('DungeonManager'), _dec2 = property(_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
        ...
        enterNextFloor() {
          ...
          var nextFloor = ((_this$_floorState$flo = (_this$_floorState = this._floorState) == null ? void 0 : _this$_floorState.floorNumber) != null ? _this$_floorState$flo : 0) + 1;
        ...
        _generateFloor(floorNumber) {
          ...
          var floorSeed = this._globalSeed + floorNumber * 1000 + this._subSeedCounter;
          this._subSeedCounter++;
          var stageDef = gm.currentStageDef;
          var isBossFloor = stageDef ? floorNumber >= stageDef.rooms : false;
          var isMiniBoss = isBossFloor && !this._isFinalBossFloor();
          var roomCount = isBossFloor ? Math.min(4, Math.max(3, this._stageRoomCount - 2)) : this._stageRoomCount;
          var dag = (_crd && DAGGenerator === void 0 ? (_reportPossibleCrUseOfDAGGenerator({
            error: Error()
        ...
        _buildRoomRuntimes(floorSeed, roomCount) {
          ...
          var builder = new (_crd && RoomBuilder === void 0 ? (_reportPossibleCrUseOfRoomBuilder({
            error: Error()
          }), RoomBuilder) : RoomBuilder)();

          for (var rl of layout.rooms) {
            var roomData = builder.build(rl);
            var nav = new (_crd && NavigationGrid === void 0 ? (_reportPossibleCrUseOfNavigationGrid({
              error: Error()
            }), NavigationGrid) : NavigationGrid)(roomData.tileMap);
            var rt = new (_crd && RoomRuntime === void 0 ? (_reportPossibleCrUseOfRoomRuntime({
              error: Error()
        ...
        _activateRoomRuntime() {
          ...
          var rt = this._roomRuntimes[this._enterCount % this._roomRuntimes.length];
          this._enterCount++;
          if (!rt) return;
        ...
        _enterRoom(roomId) {
          ...
          var room = this._floorState.dag.rooms.get(roomId);
        ...
        _onRoomEntered(room) {
          var handler = this._roomHandlers().get(room.type);
        ...
        _startBattleInRoom(room, isBoss) {
          if (isBoss === void 0) {
            isBoss = false;
          }
          ...
          var monsterConfigs = this._generateMonstersForRoom(room, isBoss);

          this.battleManager.startBattle(monsterConfigs);
        }

        _generateMonstersForRoom(room, isBoss) {
          var isElite = room.type === (_crd && RoomType === void 0 ? (_reportPossibleCrUseOfRoomType({
            error: Error()
          }), RoomType) : RoomType).Elite;
          var gm = (_crd && GameManager === void 0 ? (_reportPossibleCrUseOfGameManager({
            error: Error()
          }), GameManager) : GameManager).instance;
          var cfg = (_crd && ConfigManager === void 0 ? (_reportPossibleCrUseOfConfigManager({
            error: Error()
          }), ConfigManager) : ConfigManager).getInstance();
          var zoneId = this._currentZone;
          var monsterScale = cfg.getMonsterScale();
          var configs = [];

          if (isBoss) {
            var _this$_floorState2;

            var isFinalBoss = ((_this$_floorState2 = this._floorState) == null ? void 0 : _this$_floorState2.isMiniBossFloor) === false && this._isFinalBossFloor();

            if (isFinalBoss) {
              var bossDef = cfg.getFinalBoss(zoneId);
              ...
              if (stageDef) {
                var miniBossHP = stageDef.miniBossHP;
                configs.push({
          ...
          var monsterCount = isElite ? 2 : (_crd && MathUtils === void 0 ? (_reportPossibleCrUseOfMathUtils({
            error: Error()
          }), MathUtils) : MathUtils).randomInt(2, 4);

          for (var i = 0; i < monsterCount; i++) {
            var picked = cfg.pickMonsterFromPool(zoneId, !isElite);
        ...
        selectNextRoom(roomId) {
          ...
          var currentRoom = this._floorState.dag.rooms.get(this._floorState.currentRoomId);
          ...
          var isBattleRoom = currentRoom.type === (_crd && RoomType === void 0 ? (_reportPossibleCrUseOfRoomType({
            error: Error()
        ...
        debugCompareSeed(seed1, seed2) {
          var dag1 = (_crd && DAGGenerator === void 0 ? (_reportPossibleCrUseOfDAGGenerator({
            error: Error()
          }), DAGGenerator) : DAGGenerator).generate(seed1, 1);
          var dag2 = (_crd && DAGGenerator === void 0 ? (_reportPossibleCrUseOfDAGGenerator({
            error: Error()
        ...
        onDestroy() {
          for (var rt of this._roomRuntimes) {
