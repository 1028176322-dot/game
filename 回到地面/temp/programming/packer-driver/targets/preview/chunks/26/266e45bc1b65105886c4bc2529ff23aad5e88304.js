    ...
    execute: function () {
      ...
      _export("ResponsivePanelContent", ResponsivePanelContent = (_dec = ccclass('ResponsivePanelContent'), _dec2 = menu('UI/ResponsivePanelContent'), _dec3 = property(Node), _dec(_class = _dec2(_class = (_class2 = class ResponsivePanelContent extends Component {
        ...
        applyLayout() {
          ...
          var frame = (_this$panelFrame = this.panelFrame) != null ? _this$panelFrame : this.node.parent;
          if (!frame) return;
          var frameTrans = frame.getComponent(UITransform);
          var contentTrans = this.node.getComponent(UITransform);
          if (!frameTrans || !contentTrans) return;
          var width = Math.max(0, frameTrans.width - this.paddingX * 2);
          var height = Math.max(0, frameTrans.height - this.paddingY * 2);
