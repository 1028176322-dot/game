  ...
  return {
    ...
    execute: function () {
      ...
      _export("MainSceneController", MainSceneController = (_dec = ccclass('MainSceneController'), _dec2 = property(_crd && ShopUI === void 0 ? (_reportPossibleCrUseOfShopUI({
        ...
        _registerPanels() {
          var router = (_crd && UiRouter === void 0 ? (_reportPossibleCrUseOfUiRouter({
            error: Error()
          }), UiRouter) : UiRouter).instance;
          var candidates = ['LoginPanel', 'CreatePanel', 'CharacterPanel', 'AreaSelectPanel', 'SettlementPanel', 'SettingsPanel', 'AdventureLogPanel']; // Search from scene root (panels are children of Canvas, not of MainSceneController)

          var sceneRoot = this.node.scene;

          for (var name of candidates) {
            // Search from scene root, recursively
            var node = this._findChildRecursive(sceneRoot != null ? sceneRoot : this.node, name);
        ...
        _findChildRecursive(parent, name) {
          ...
          for (var child of parent.children) {
            var found = this._findChildRecursive(child, name);
        ...
        _ensurePersistentMainBackground() {
          ...
          var canvas = this._findChildRecursive((_this$node$scene2 = this.node.scene) != null ? _this$node$scene2 : this.node, 'Canvas');
          ...
          var bg = canvas.getChildByName('MainBackground');
          ...
          var backdrop3D = canvas.getChildByName('MainMenuBackdrop3D');

          if (!backdrop3D) {
            backdrop3D = new Node('MainMenuBackdrop3D');
            backdrop3D.layer = canvas.layer;
            var ui = backdrop3D.addComponent(UITransform);
        ...
        _onFlowState(state) {
          ...
          var mainUI = (_this$node$parent$get = (_this$node$parent = this.node.parent) == null ? void 0 : _this$node$parent.getChildByName('MainUI')) != null ? _this$node$parent$get : this._findChildRecursive((_this$node$scene3 = this.node.scene) != null ? _this$node$scene3 : this.node, 'MainUI');
          var showMainUI = state === (_crd && AppFlowState === void 0 ? (_reportPossibleCrUseOfAppFlowState({
            error: Error()
        ...
        _bindFallbackStartButton() {
          ...
          if (this._startButton) {
            var existing = this._startButton;
          ...
          var startNode = (_ref = (_parentNode$getChildB = parentNode == null ? void 0 : parentNode.getChildByName('StartButton')) != null ? _parentNode$getChildB : this.node.getChildByName('StartButton')) != null ? _ref : this._findStartButtonInScene();
          var startButton = (_startNode$getCompone = startNode == null ? void 0 : startNode.getComponent(Button)) != null ? _startNode$getCompone : null;
        ...
        _bindStartButton(button) {
          var node = button == null ? void 0 : button.node;
          if (!node || !node.isValid) return;
          node.on(Button.EventType.CLICK, this._onOpenAreaSelect, this);
        }

        _unbindStartButton(button) {
          var _button;

          if (button === void 0) {
            button = this._startButton;
          }

          var node = (_button = button) == null ? void 0 : _button.node;
