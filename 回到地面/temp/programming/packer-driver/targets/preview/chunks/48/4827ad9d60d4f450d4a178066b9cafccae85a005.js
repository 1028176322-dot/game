    ...
    execute: function () {
      ...
      _export("RuntimeEntityFactory", RuntimeEntityFactory = class RuntimeEntityFactory {
        static create(spec) {
          ...
          var root = new Node(spec.name);
          this.ensureTransform(root, (_spec$width = spec.width) != null ? _spec$width : 96, (_spec$height = spec.height) != null ? _spec$height : 96);
          var bodySpec = (_spec$body = spec.body) != null ? _spec$body : {};
          var body = this.ensureSpriteChild(root, (_bodySpec$name = bodySpec.name) != null ? _bodySpec$name : 'Body', (_ref = (_bodySpec$width = bodySpec.width) != null ? _bodySpec$width : spec.width) != null ? _ref : 96, (_ref2 = (_bodySpec$height = bodySpec.height) != null ? _bodySpec$height : spec.height) != null ? _ref2 : 96);
          var slots = {};

          for (var slot of (_spec$slots = spec.slots) != null ? _spec$slots : []) {
            var _spec$slots, _slot$width, _slot$height, _slot$x, _slot$y;

            var child = this.ensureChild(root, slot.name, (_slot$width = slot.width) != null ? _slot$width : 1, (_slot$height = slot.height) != null ? _slot$height : 1);
          ...
          if ((_spec$hpBar = spec.hpBar) != null && _spec$hpBar.enabled) {
            ...
            var hpBar = this.createHPBar(root, (_spec$hpBar$width = spec.hpBar.width) != null ? _spec$hpBar$width : 84, (_spec$hpBar$height = spec.hpBar.height) != null ? _spec$hpBar$height : 12);
          ...
          if ((_spec$shadow = spec.shadow) != null && _spec$shadow.enabled) {
            ...
            var shadow = this.ensureChild(root, 'Shadow', (_spec$shadow$width = spec.shadow.width) != null ? _spec$shadow$width : 70, (_spec$shadow$height = spec.shadow.height) != null ? _spec$shadow$height : 20);
        ...
        static ensureSpriteChild(parent, name, width, height) {
          var child = this.ensureChild(parent, name, width, height);
        ...
        static createHPBar(parent, width, height) {
          var hpBar = this.ensureChild(parent, 'HPBar', width, height);
          var bg = this.ensureChild(hpBar, 'BarBg', width, Math.max(2, height - 2));
          this.drawRect(bg, width, Math.max(2, height - 2), new Color(35, 35, 35, 190));
          var fill = this.ensureChild(hpBar, 'BarFill', Math.max(2, width - 4), Math.max(2, height - 4));
        ...
        static drawRect(node, width, height, color) {
          ...
          var graphics = (_node$getComponent2 = node.getComponent(Graphics)) != null ? _node$getComponent2 : node.addComponent(Graphics);
          graphics.clear();
