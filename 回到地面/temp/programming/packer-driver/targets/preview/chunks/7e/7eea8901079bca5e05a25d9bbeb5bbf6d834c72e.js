    ...
    execute: function () {
      ...
      _export("RoomBuilder", RoomBuilder = class RoomBuilder {
        ...
        build(layout) {
          ...
          var tileMap = new (_crd && TileMap === void 0 ? (_reportPossibleCrUseOfTileMap({
            error: Error()
          }), TileMap) : TileMap)(width, height, 'floor');
          var pieces = [];
          var assetSet = new Set();
          var floorAsset = "tile/" + layoutZone(layout.type) + "/floor";
          var wallAsset = "tile/" + layoutZone(layout.type) + "/wall";
          ...
          if (layout.type !== 'start' && width >= 3 && height >= 3) {
            var cx = width / 2 | 0;
            var cy = height / 2 | 0;
            var decoAsset = "decoration/" + layout.type;
            pieces.push({
              kind: 'decoration',
              assetId: decoAsset,
              x: cx,
