System.register(["cc"], function (_export, _context) {
  ...
  function resolveSocketByName(root, name, fallback) {
    var want = normalizeSocketName(name);
    var fall = fallback ? normalizeSocketName(fallback) : null;
    var fallbackHit = null;
    var stack = [root];

    while (stack.length > 0) {
      var cur = stack.pop();
      var n = normalizeSocketName(cur.name);
      if (n === want) return cur;
      if (fall && n === fall) fallbackHit = cur;

      for (var child of cur.children) stack.push(child);
    }
