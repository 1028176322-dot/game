  ...
  return {
    ...
    execute: function () {
      ...
      _export("CombatSystem", CombatSystem = class CombatSystem {
        ...
        unregister(entityId) {
          var idx = this._pool.findIndex(e => e.id === entityId);
        ...
        dispatch(cmd) {
          ...
          var self = this._pool.find(e => e.id === cmd.entityId);
          ...
          var result = selector.resolve(cmd, this._pool, self);

          this._lockOn.apply(result);

          var handler = this._handlers.get(cmd.kind);
        ...
        update(dt) {
          // Advance projectiles.
          var hits = this._projectiles.update(dt, this._pool);
        ...
        _handleSkill(cmd, self, _pool, result) {
          ...
          var skillId = (_cmd$skillId = cmd.skillId) != null ? _cmd$skillId : 'melee_attack';

          var skillConfig = this._configDB.getSkill(skillId);

          var data = skillConfig;
          ...
          var nodes = this._skillGraph.build(data);

          var aim = (_cmd$aimPosition = cmd.aimPosition) != null ? _cmd$aimPosition : {
            x: self.gridX,
            y: 0,
            z: self.gridY
          };
          ...
          if (result.primary) {
            ...
            var dmg = (_data$onHit$damage = (_data$onHit = data.onHit) == null ? void 0 : _data$onHit.damage) != null ? _data$onHit$damage : 0;
        ...
        _applyProjectileHit(hit) {
          ...
          var target = this._pool.find(e => e.id === hit.targetId);
