    ...
    execute: function () {
      ...
      _export("BattleManager", BattleManager = (_dec = ccclass('BattleManager'), _dec2 = property(Prefab), _dec(_class = (_class2 = class BattleManager extends Component {
        ...
        startBattle(monsterConfigs) {
          ...
          for (var config of monsterConfigs) {
            var spawnPos = this._findSpawnPosition();
        ...
        _clearMonsters() {
          for (var entry of this._monsters) {
            var _entry$monster;
        ...
        _spawnMonster(config, gridX, gridY, isSummon) {
          if (isSummon === void 0) {
            isSummon = false;
          }
          ...
          var prefab = !isSummon ? this.monsterPrefab : null;
          var runtime = prefab ? this._createFromPrefab(config, gridX, gridY, prefab) : (_crd && MonsterRuntimeFactory === void 0 ? (_reportPossibleCrUseOfMonsterRuntimeFactory({
            error: Error()
          }), MonsterRuntimeFactory) : MonsterRuntimeFactory).create("monster_" + gridX + "_" + gridY);
          var monsterNode = runtime.root;

          this._actorLayer.addChild(monsterNode);

          var pos = this._gridManager.gridToWorld(gridX, gridY);
        ...
        _applyMonsterVisual(monsterNode, config) {
          var _this = this;

          return _asyncToGenerator(function* () {
            if (!config.zoneId || !config.id) return;
            var bodyNode = (_crd && MonsterRuntimeFactory === void 0 ? (_reportPossibleCrUseOfMonsterRuntimeFactory({
              error: Error()
            ...
            if (config.visual) {
              var _visualOk = yield (_crd && CharacterVisualService === void 0 ? (_reportPossibleCrUseOfCharacterVisualService({
                error: Error()
              }), CharacterVisualService) : CharacterVisualService).instance.applyStatic(bodyNode, config.visual);

              if (_visualOk) {
                _this._scaleMonster(monsterNode, config);
            ...
            var visualKey = "monster." + config.zoneId + "." + config.id.toLowerCase();
            var visualOk = yield (_crd && CharacterVisualService === void 0 ? (_reportPossibleCrUseOfCharacterVisualService({
              error: Error()
        ...
        _scaleMonster(monsterNode, config) {
          var transform = monsterNode.getComponent(UITransform);
        ...
        _findSpawnPosition() {
          ...
          var gridSize = this._gridManager.gridSize;
          var playerX = (_this$_player$gridX = (_this$_player = this._player) == null ? void 0 : _this$_player.gridX) != null ? _this$_player$gridX : Math.floor(gridSize / 2);
          var playerY = (_this$_player$gridY = (_this$_player2 = this._player) == null ? void 0 : _this$_player2.gridY) != null ? _this$_player$gridY : Math.floor(gridSize / 2);
          ...
          for (var _y = 0; _y < gridSize; _y++) {
            for (var _x = 0; _x < gridSize; _x++) {
              if (this._gridManager.isWalkable(_x, _y) && !this._gridManager.isOccupied(_x, _y)) {
                return {
                  x: _x,
                  y: _y
                };
        ...
        getNearestMonster(position, range) {
          ...
          var nearest = null;
          var nearestDist = range * (_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
            error: Error()
          }), GameConfig) : GameConfig).TILE_SIZE + 1;

          for (var entry of this._monsters) {
            var node = entry.monster.node;
            if (!node || !node.isValid || entry.monster.isDead) continue;
            var dist = Vec3.distance(position, node.getPosition());
        ...
        removeMonster(monster) {
          ...
          var idx = this._monsters.findIndex(e => e.monster === monster);
