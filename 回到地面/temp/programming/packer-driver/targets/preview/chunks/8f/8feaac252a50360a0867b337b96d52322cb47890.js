  ...
  return {
    ...
    execute: function () {
      ...
      _export("DAGGenerator", DAGGenerator = class DAGGenerator {
         ...
         */
        static generate(seed, floorNumber, roomCount, options) {
          if (roomCount === void 0) {
            roomCount = 5;
          }

          var rand = (_crd && MathUtils === void 0 ? (_reportPossibleCrUseOfMathUtils({
            error: Error()
          }), MathUtils) : MathUtils).seededRandom(seed);
          var rooms = new Map();
          var nextId = 0;
          var isBossFloor = (options == null ? void 0 : options.isMiniBossFloor) || (options == null ? void 0 : options.isFinalBossFloor); // ======== 第 1 层：入口 ========
          ...
          var depthCount = isBossFloor ? Math.max(1, roomCount - 3) // Boss层：入口→战斗→强化→Boss
          : roomCount - 1; // 普通层：去掉Boss层和强化层

          var prevDepthRooms = [entryId];

          for (var depth = 1; depth <= depthCount; depth++) {
            var currentDepthRooms = [];
            var roomsThisDepth = depth === depthCount ? 1 : rand() < 0.6 ? 1 : 2;

            for (var i = 0; i < roomsThisDepth; i++) {
              var roomId = nextId++;

              var roomType = this._determineRoomType(rand, depth, depthCount, i, options);
              ...
              for (var prevId of prevDepthRooms) {
          ...
          if (isBossFloor) {
            ...
            if (prevDepthRooms.length > 0) {
              var _upgradeId = nextId++;

              rooms.set(_upgradeId, {
                id: _upgradeId,
                type: (_crd && RoomType === void 0 ? (_reportPossibleCrUseOfRoomType({
                  error: Error()
              ...
              for (var _prevId of prevDepthRooms) {
                rooms.get(_prevId).connections.push(_upgradeId);
              }

              prevDepthRooms = [_upgradeId];
            } // ======== 末端：Boss 房 ========
            ...
            if (prevDepthRooms.length > 0) {
              rooms.get(prevDepthRooms[0]).connections.push(_bossId);
            }
          ...
          for (var _prevId2 of prevDepthRooms) {
            rooms.get(_prevId2).connections.push(upgradeId);
        ...
        static _determineRoomType(rand, depth, maxDepth, branchIndex, options) {
          ...
          var weights = options != null && options.zoneId ? (_crd && ConfigManager === void 0 ? (_reportPossibleCrUseOfConfigManager({
            error: Error()
          ...
          var roll = rand() * 100; // 深度 > 2 时可以出精英房
