    ...
    execute: function () {
      ...
      _export("CombatEffectService", CombatEffectService = class CombatEffectService {
        ...
        _findCamera() {
          ...
          var cams = (_director$getScene = director.getScene()) == null ? void 0 : _director$getScene.getComponentsInChildren(Camera);
        ...
        _playAnim(action, loop, fps) {
          ...
          var key = "character." + this._characterClass + "." + action;
        ...
        _screenShake(intensity, duration) {
          ...
          var orig = this._cameraOrig;
          var steps = Math.floor(duration / 0.03);
          ...
          var _loop = function _loop(i) {
            ...
            setTimeout(() => {
              ...
              if (!((_this$_cameraNode3 = _this._cameraNode) != null && _this$_cameraNode3.isValid)) return;
              var decay = 1 - i / steps;
              var ox = (Math.random() - 0.5) * intensity * decay;
              var oy = (Math.random() - 0.5) * intensity * decay;

              _this._cameraNode.setPosition(orig.x + ox, orig.y + oy, orig.z);
            }, t);
          };
          ...
          setTimeout(() => {
            ...
            if ((_this$_cameraNode2 = this._cameraNode) != null && _this$_cameraNode2.isValid) this._cameraNode.setPosition(orig);
          }, duration * 1000 + 50);
        }
        ...
        _spawnProjectile(from, to, isCrit) {
          ...
          if (!canvas) return;
          var proj = new Node('_projectile');
          var uiTransform = proj.addComponent(UITransform);
          var size = isCrit ? 32 : 20;
          uiTransform.setContentSize(size, size);
          var sprite = proj.addComponent(Sprite);
          ...
          var mid = new Vec3((from.x + to.x) * 0.5, (from.y + to.y) * 0.5 + 20, 0);
          tween(proj).to(0.08, {
            position: mid
          }).to(0.06, {
            position: to
