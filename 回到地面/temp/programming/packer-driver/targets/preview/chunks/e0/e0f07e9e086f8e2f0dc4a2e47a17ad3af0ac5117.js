    ...
    execute: function () {
      ...
      _export("DungeonMapUI", DungeonMapUI = (_dec = ccclass('DungeonMapUI'), _dec2 = property(Prefab), _dec3 = property(Node), _dec4 = property(Sprite), _dec(_class = (_class2 = class DungeonMapUI extends Component {
        ...
        _renderMap(dag) {
          ...
          for (var [depth, rooms] of depthMap) {
            var nodeX = marginX + depth * (roomWidth + 10);
            var roomsAtDepth = rooms.length;

            for (var i = 0; i < roomsAtDepth; i++) {
              var _room = rooms[i];
              var nodeY = marginY + (roomsAtDepth > 1 && i === 1 ? roomHeight + 8 : 0);

              var node = this._createRoomNode(_room, nodeX, nodeY, roomWidth, roomHeight);

              this.mapContainer.addChild(node);

              this._nodeMap.set(_room.id, node);
            }
          } // 渲染连接线（简化：在节点间画斜线）
        ...
        _createRoomNode(room, x, y, w, h) {
          ...
          if (this.roomNodePrefab) {
            ...
            var _sprite = node.addComponent(Sprite);
          }
          ...
          if (sprite) {
            var color = this._roomColors[room.type] || new Color(180, 180, 180);
            sprite.color = color;
          } // 标签
        ...
        _updateNodeHighlight(roomId) {
          // 取消旧高亮
          for (var [, node] of this._nodeMap) {
            node.setScale(1, 1, 1);
          } // 新高亮


          var currentNode = this._nodeMap.get(roomId);
        ...
        _markRoomCleared(roomId) {
          var node = this._nodeMap.get(roomId);
