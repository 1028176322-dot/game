    ...
    execute: function () {
      ...
      _export("ResponsivePanelRoot", ResponsivePanelRoot = (_dec = ccclass('ResponsivePanelRoot'), _dec2 = menu('UI/ResponsivePanelRoot'), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property({
        ...
        onEnable() {
          ...
          if (this.dimMask) {
            var sprite = this.dimMask.getComponent(Sprite);
            if (sprite && !sprite.enabled) sprite.enabled = true;
          }

          if (this.panelFrame) {
            var _sprite = this.panelFrame.getComponent(Sprite);

            if (_sprite && !_sprite.enabled) _sprite.enabled = true;
          }
        ...
        applyLayout() {
          ...
          var canvasW = visible.width;
          var canvasH = visible.height;
          var parent = this.node.parent;
          var parentTrans = parent == null ? void 0 : parent.getComponent(UITransform);
          ...
          if (this.dimMask) {
            var maskTrans = this.dimMask.getComponent(UITransform);
          ...
          if (this.panelFrame) {
            var frameTrans = this.panelFrame.getComponent(UITransform);

            if (frameTrans) {
              var w = clamp(canvasW * this.frameWidthRatio, this.minFrameWidth, this.maxFrameWidth);
              var h = clamp(canvasH * this.frameHeightRatio, this.minFrameHeight, this.maxFrameHeight);
              frameTrans.setContentSize(w, h);
            }

            this.panelFrame.setPosition(Vec3.ZERO); // 触发 ContentRoot 重新布局

            var content = this.panelFrame.getChildByName('ContentRoot');

            if (content) {
              var contentComp = content.getComponent(_crd && ResponsivePanelContent === void 0 ? (_reportPossibleCrUseOfResponsivePanelContent({
                error: Error()
