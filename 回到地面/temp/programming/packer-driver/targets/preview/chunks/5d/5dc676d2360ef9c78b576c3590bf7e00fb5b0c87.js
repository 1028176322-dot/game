      ...
      _export("ItemSystem", ItemSystem = (_dec = ccclass('ItemSystem'), _dec(_class = class ItemSystem extends Component {
        ...
        pickupItem(itemDef, count) {
          ...
          for (var i = 0; i < this._bag.length; i++) {
            var stack = this._bag[i];

            if (stack && stack.def.id === itemDef.id && stack.count < itemDef.stackMax) {
              var addable = Math.min(itemDef.stackMax - stack.count, count);
              stack.count += addable;
              count -= addable;
          ...
          for (var _i = 0; _i < this._bag.length; _i++) {
            if (this._bag[_i] === null) {
              var addCount = Math.min(count, itemDef.stackMax);
              this._bag[_i] = {
                def: _extends({}, itemDef),
                count: addCount
              };
              count -= addCount;
            ...
            case 'aoe':
              {
                ...
                var radius = ((_itemDef$aoeRange = itemDef.aoeRange) != null ? _itemDef$aoeRange : 2) * (_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
                  error: Error()
                ...
                for (var m of monsters) {
                  ...
                  if (d <= radius) {
                    // AoE 伤害
                    if (itemDef.damage) {
                      var killed = m.takeDamage(itemDef.damage, false);
