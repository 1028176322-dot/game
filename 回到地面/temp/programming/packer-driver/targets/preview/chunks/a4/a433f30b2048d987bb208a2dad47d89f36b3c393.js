  ...
  return {
    ...
    execute: function () {
      ...
      TypedEmitterImpl = class TypedEmitterImpl {
        constructor(domainName, logEnabled) {
        ...
        subscribe(type, handler) {
          ...
          var list = (_this$_handlers$get = this._handlers.get(type)) != null ? _this$_handlers$get : [];
          list.push(handler);
        ...
        emit(event) {
          var list = this._handlers.get(event.type);

          if (!list) return;

          for (var _handler of list) {
            try {
              _handler(event);
            } catch (err) {
          ...
          if (this._logEnabled) {
            ...
            var _logger = (_this$_logger = this._logger) != null ? _this$_logger : console.log;

            _logger("[EventBus] " + this._domainName + "/" + event.type);
          }
        }
      ...
      _export("EventBusManager", EventBusManager = class EventBusManager {
        ...
        setDomainLog(domain, enabled, logger) {
          var emitter = this._domains.get(domain);
        ...
        clearAll() {
          for (var emitter of this._domains.values()) {
            emitter.clear();
          }
        } // --- ILifecycle (§5.1) ---
