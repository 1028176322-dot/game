    ...
    execute: function () {
      ...
      _export("StorageService", StorageService = class StorageService {
        ...
        getJson(key, defaultValue) {
          try {
            var raw = this._adapter.getString(key);
        ...
        restoreBackup(key, defaultValue) {
          var backupRaw = this._adapter.getString(key + "_backup");

          var backupKey = key + "_backup";

          if (backupRaw !== null) {
            try {
              var parsed = JSON.parse(backupRaw); // Restore backup to primary key
        ...
        readWithMigration(key, defaultValue, migrations) {
          ...
          var result = this.getJson(key, defaultValue);

          if (!result.ok || result.source === 'default') {
            // Try backup recovery before giving up
            if (result.error) {
              var recovered = this.restoreBackup(key, defaultValue);
          ...
          var currentVersion = (_current$schemaVersio = current.schemaVersion) != null ? _current$schemaVersio : 1;
          var versions = Object.keys(migrations).map(Number).filter(v => !isNaN(v)).sort((a, b) => a - b);
          var migrated = false;

          for (var targetVer of versions) {
            if (targetVer > currentVersion) {
              var migrateFn = migrations[targetVer];
