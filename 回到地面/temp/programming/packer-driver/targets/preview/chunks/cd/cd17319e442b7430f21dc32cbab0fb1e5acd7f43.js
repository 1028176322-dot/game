    ...
    execute: function () {
      ...
      _export("EquipmentView", EquipmentView = (_dec = ccclass('EquipmentView'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Node), _dec(_class = (_class2 = class EquipmentView extends Component {
        ...
        buildFallbackLayout() {
          ...
          for (var _slot of _crd && EQUIPMENT_SLOTS === void 0 ? (_reportPossibleCrUseOfEQUIPMENT_SLOTS({
            error: Error()
          }), EQUIPMENT_SLOTS) : EQUIPMENT_SLOTS) {
            var node = this._buildSlotNode(_slot, slotPositions[_slot]);

            panel.addChild(node.root);

            this._slotNodes.set(_slot, node);
          } // 12 格背包
          ...
          for (var i = 0; i < 12; i++) {
            var col = i % 3,
                row = Math.floor(i / 3);
            var x = bpStartX + col * (cell + pad),
                y = bpStartY - row * (cell + pad);

            var _node = this._buildBackpackCell(i, x, y);

            panel.addChild(_node.root);

            this._backpackNodes.push(_node);
          } // 套装信息


          var siN = new Node('setInfo');
        ...
        renderSlots(slots) {
          for (var s of slots) {
            var node = this._slotNodes.get(s.slot);
        ...
        renderBackpack(backpack) {
          for (var i = 0; i < this._backpackNodes.length && i < backpack.length; i++) {
            var _RARITY_COLORS$item$r;

            var node = this._backpackNodes[i];
            var item = backpack[i].item;
        ...
        _buildSlotNode(slot, pos) {
          ...
          var root = new Node("slot_" + slot);
          root.setPosition(pos);
          var t = root.addComponent(UITransform);
          ...
          root.addChild(bg);
          var nameN = new Node('name');
          root.addChild(nameN);
          var label = nameN.addComponent(Label);
          label.fontSize = 14;
          label.color = Color.WHITE;
          var slotN = new Node('slotName');
          slotN.setPosition(0, -16);
          root.addChild(slotN);
          var sLabel = slotN.addComponent(Label);
          sLabel.string = (_slot2 = (_crd && SLOT_NAMES === void 0 ? (_reportPossibleCrUseOfSLOT_NAMES({
            error: Error()
          }), SLOT_NAMES) : SLOT_NAMES)[slot]) != null ? _slot2 : slot;
          sLabel.fontSize = 12;
