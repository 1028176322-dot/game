  ...
  return {
    ...
    execute: function () {
      ...
      _export("AudioSystem", AudioSystem = class AudioSystem {
        ...
        setSnapshot(name) {
          var s = this._snapshots.get(name);
        ...
        destroy() {
          for (var _id of this._playing) this._sink.stop(_id);
        ...
        _play(id, category, worldPos) {
          var cfg = this._resolveConfig(id, category);

          var volume = cfg.volume * this._volumeScale;

          if (category === AudioCategory.Spatial && worldPos) {
            volume *= this._attenuation(worldPos, cfg);
          }

          this._sink.play(_extends({}, cfg, {
            category,
            volume
          }), volume);
        ...
        _resolveConfig(id, category) {
          ...
          var raw = this._db ? this._db.getAudio(id) : undefined;
          return {
            id,
            category,
        ...
        _attenuation(pos, cfg) {
          var min = cfg.minDistance;
          var max = Math.max(cfg.minDistance + 0.0001, cfg.maxDistance);
          var dx = pos.x - this._listener.x;
          var dy = pos.y - this._listener.y;
          var dz = pos.z - this._listener.z;
