  ...
  return {
    ...
    execute: function () {
      ...
      _export("ArtResourceResolver", ArtResourceResolver = class ArtResourceResolver {
        static character(characterId, action) {
          if (action === void 0) {
            action = 'idle';
          }

          var id = this._compact(characterId);

          return "textures/characters/" + id + "/" + id + "_" + action;
        }

        static monster(zoneId, monsterId, action) {
          if (action === void 0) {
            action = 'idle';
          }

          var zone = this._compact(zoneId);

          var monster = this._compact(monsterId);
        ...
        static tile(zoneId, terrain) {
          ...
          var kind = this.tileKind(terrain);
