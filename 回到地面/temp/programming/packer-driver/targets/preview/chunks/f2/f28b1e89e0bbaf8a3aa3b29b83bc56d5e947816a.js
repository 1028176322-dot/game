      ...
      _export("ShopView", ShopView = (_dec = ccclass('ShopView'), _dec2 = property(Node), _dec(_class = (_class2 = class ShopView extends Component {
        ...
        buildFallbackLayout() {
          ...
          var _loop = function _loop() {
            ...
            var idx = i;
            tab.on(Node.EventType.TOUCH_END, () => _this._onTabClick == null ? void 0 : _this._onTabClick("tab_" + idx));
          };
          ...
          var closeBtn = this._addLabel(panel, 'closeBtn', 190, 170, 'X', 16, Color.RED);
        ...
        render(vm) {
          ...
          for (var i = 0; i < this._tabs.length; i++) {
            ...
            var label = this._tabs[i].getComponent(Label);
        ...
        _buildItem(item) {
          var root = new Node("item_" + item.id);
          root.addComponent(UITransform).setContentSize(400, 40);
          var y = -this._items.length * 45;
          root.setPosition(0, y);

          var nameL = this._addLabel(root, 'name', -160, 0, item.name, 13, Color.WHITE);

          var descL = this._addLabel(root, 'desc', -30, 0, item.description, 10, new Color(180, 180, 180));

          var costL = this._addLabel(root, 'cost', 130, 0, "" + item.cost, 12, item.canAfford ? Color.YELLOW : Color.RED);

          var actionL = this._addLabel(root, 'action', 180, 0, item.actionLabel, 12, item.canAfford ? Color.GREEN : Color.GRAY);
