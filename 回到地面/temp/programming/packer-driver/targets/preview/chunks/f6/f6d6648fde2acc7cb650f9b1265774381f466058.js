    ...
    execute: function () {
      ...
      _export("EventSystem", EventSystem = class EventSystem {
        ...
        generateEvent(currentFloor, zoneId) {
          var scene = this._pickScene();

          var description = this._buildDescription(scene);

          var options = this._generateOptions(scene);
        ...
        _pickScene() {
          ...
          var totalWeight = EVENT_SCENES.reduce((s, sc) => s + sc.weight, 0);
          var roll = rng.next() * totalWeight;

          for (var scene of EVENT_SCENES) {
            roll -= scene.weight;
        ...
        _generateOptions(scene) {
          ...
          if (scene.specialConditions && this._player) {
            for (var cond of scene.specialConditions) {
        ...
        _checkCondition(check) {
          ...
          var stats = this._player.stats.getFinalStats();

          var hpRatio = stats.maxHP > 0 ? this._player.currentHP / stats.maxHP : 0;
        ...
        _getDefaultOptions(scene) {
          ...
          var defaultRng = (_crd && RunRng === void 0 ? (_reportPossibleCrUseOfRunRng({
            error: Error()
          }), RunRng) : RunRng).instance.fork('event:defaultOptions');
          var pick = defaultOptions[defaultRng.int(0, defaultOptions.length - 1)];
          return {
            optionA: {
              label: pick.labelA,
