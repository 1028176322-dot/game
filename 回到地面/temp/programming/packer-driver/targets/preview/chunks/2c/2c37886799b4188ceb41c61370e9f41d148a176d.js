    ...
    execute: function () {
      ...
      _export("DungeonGenerator", DungeonGenerator = class DungeonGenerator {
        ...
        generate(seed, zone, opts) {
          ...
          var count = clamp((_opts$roomCount = opts.roomCount) != null ? _opts$roomCount : 6, 3, 12);
          var minSize = (_opts$minSize = opts.minSize) != null ? _opts$minSize : 6;
          var maxSize = (_opts$maxSize = opts.maxSize) != null ? _opts$maxSize : 10;
          var rooms = []; // Spine along +X. Each room gets a deterministic interior size.

          for (var i = 0; i < count; i++) {
            var type = pickType(i, count);
            rooms.push({
              id: "r" + i,
              type,
              gridX: i,
