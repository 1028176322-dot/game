    ...
    execute: function () {
      ...
      _export("AutoAttack", AutoAttack = (_dec = ccclass('AutoAttack'), _dec(_class = (_class2 = class AutoAttack extends Component {
        ...
        update(dt) {
          ...
          var interval = finalStats.atkSpeed;
          this._timer += dt;
        ...
        _performAutoAttack(stats) {
          ...
          var nearestMonster = this._battleManager.getNearestMonster(this.node.getPosition(), stats.attackRange);
          ...
          var target = nearestMonster.monster;
          ...
          var distInTiles = dist / (_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
            error: Error()
          ...
          var preAttack = this.onBeforeAttack ? this.onBeforeAttack() : null;
          var isCrit = (preAttack == null ? void 0 : preAttack.forceCrit) || (_crd && MathUtils === void 0 ? (_reportPossibleCrUseOfMathUtils({
            error: Error()
          }), MathUtils) : MathUtils).chance(stats.critChance);
          var rawDamage = isCrit ? Math.floor(stats.atk * stats.critMultiplier) : stats.atk; // 应用伤害倍率

          var finalDamage = Math.max((_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
            error: Error()
          }), GameConfig) : GameConfig).MIN_DAMAGE, Math.floor(rawDamage * stats.damageMultiplier)); // 施加伤害

          var killed = target.takeDamage(finalDamage, isCrit); // 生命偷取

          if (stats.lifeSteal > 0 && finalDamage > 0) {
            var healAmount = Math.max((_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
              error: Error()
