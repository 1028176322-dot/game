      ...
      _export("SkillExecutor", SkillExecutor = class SkillExecutor {
        ...
        _registerHandlers() {
          ...
          this._handlers.set('explosion', (node, rt) => {
            var n = node;
            var hits = rt.collision.overlapSphere(rt.aim, n.radius);
        ...
        execute(data, caster, aim) {
          ...
          var nodes = this._graph.build(data);
          ...
          for (var _node of nodes) {
            var handler = this._handlers.get(_node.kind);

            if (!handler) {
              this._logger.channel('battle').warn("[Skill] no handler registered for node kind: " + _node.kind);
