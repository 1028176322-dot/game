...
import { PlayerProfileSave } from './SaveTypes';
...
export class SaveValidator {
    ...
    static validateProfile(profile: PlayerProfileSave): ValidationResult {
        const errors: string[] = [];
        ...
        if (!profile.profile.unlockedCharacters.includes(profile.profile.selectedCharacter)) {
            errors.push(
                `selectedCharacter "${profile.profile.selectedCharacter}" is not in unlockedCharacters [${
                    profile.profile.unlockedCharacters.join(', ')
                }] — resetting to "warrior"`,
            );
        }
    ...
    static validateBeforeSave(profile: PlayerProfileSave): ValidationResult {
        const errors: string[] = [];
        ...
        if (!profile.profile.selectedCharacter &&
            profile.profile.unlockedCharacters.length > 0) {
    ...
    static repairProfile(profile: PlayerProfileSave): { fixed: boolean; changes: string[] } {
        ...
        if (!profile.profile.unlockedCharacters.includes('warrior')) {
            ...
            profile.profile.unlockedCharacters = [
                'warrior',
                ...profile.profile.unlockedCharacters.filter(id => id !== 'warrior'),
            ];
            fixed = true;
        }
