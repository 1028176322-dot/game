...
export class PlayerController extends Component {
    ...
    takeDamage(rawDamage: number, isCrit: boolean = false): void {
        ...
        runEvents.emit('player:damaged', { amount: actualDamage, isCrit });
    ...
    private _calcDamage(rawDamage: number, stats: { def: number; damageReduction: number }): number {
        const dmgRng = RunRng.instance.fork('player:damageCalc');
    ...
    heal(amount: number): void {
        ...
        runEvents.emit('player:healed', { amount: effectiveAmount });
    }
