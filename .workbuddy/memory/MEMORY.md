...
- **ART_GENERATION_PIPELINE - 2026-07-06**: Before any AI art resource generation (new/replace/crop/compress/import), MUST read `docs/美术资源生成与入库规范.md` first. Follow the full pipeline in Section 9 (prompts.json → generate master → review → runtime candidate → copy → register → validate → preview). Use `@skill:agnes-image-gen` exclusively for image generation. All generated resources must comply with the format/size/budget tables in Section 4 and the safety rules throughout the spec.
