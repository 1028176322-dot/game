...
- **SKILL_REFERENCE - 2026-07-08**: `docs/SKILL_REFERENCE.md` is the mandatory-read index of all available Skills. **Always check if the current task matches any Skill's trigger condition** before beginning. MANDATORY-level Skills must be invoked for their corresponding operations.
...
- **ART_GENERATION_PIPELINE - 2026-07-08**: Before any art resource creation/replacement/crop/compress/import -- ANY operation that touches `assets/resources/textures/` -- the `art-pipeline` skill MUST be invoked first via `Skill({ skill: "art-pipeline" })`. Follow the full 12-step pipeline: confirm specs -> generate -> master -> runtime_candidate -> technical_check -> backup (to art_source/textures_review/backup/, NEVER to assets/resources/textures/) -> import -> register -> validate -> visual_review -> approve. For AI images, first read `assets/resources/config/prompts.json` and update it before calling the API; for procedural UI panels, use the bundled `scripts/generate_panel.py`. After ANY resource change, always run `npm.cmd run validate:all`. Backup files generated by previous sessions have been cleaned from the project directory. All `.bak` files placed in `assets/resources/textures/` are forbidden going forward.
...
2. **`docs/SKILL_REFERENCE.md`.** Read immediately after this file -- check if current task matches any Skill's trigger condition.
...
| Skill | Level | Trigger Summary |
|---|---|---|
| `art-pipeline` | **MANDATORY** | Any art resource operation + AI image gen (merged agnes-image-gen) |
| `encoding-pipeline-guard` | **MANDATORY** | Any source file (TS/Python/JSON/MD) creation/editing |
| `ardot-mindmap-game-system-flow` | On-demand | Game system flow diagram / mind map |
