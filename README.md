# 回到地面 (BackToGround)

实时自动攻击 · 战术 Roguelike 地牢探险

> **引擎**: Cocos Creator 3.x + TypeScript
> **平台**: 微信小游戏 (IAA 纯广告变现)
> **状态**: Phase 0 (设计阶段) · v0.5 · 10 项规格已完成
> **编码**: UTF-8
> **仓库**: [github.com/1028176322-dot/game](https://github.com/1028176322-dot/game)
> **分支**: `main` (稳定) / `develop` (开发) / `feature/*` (功能)

---

## 项目简介

一款**实时自动攻击**的 Roguelike 地牢探险游戏。玩家操控角色深入随机生成的地牢，角色自动攻击敌人，玩家专注于走位、翻滚闪避和技能时机的判断。每层选择强化构筑 Build，触发元素反应组合，直到倒地或触及最深处——然后"回到地面"，重头再来。

**设计支柱**: 走位决策重于手速 · 每次旅程都不同 · 短平快节奏(5-15min) · 垃圾时间也有价值

## 快速入口

| 文件 | 说明 | 版本 |
|------|------|------|
| [GDD 主设计文档](./GDD_回到地面.md) | 项目入口，全局概览 | v0.1(已同步) |
| [主体方案确定](./design/主体方案确定.md) | 所有设计决策记录 | v0.5 |
| [主体系统架构](./design/主体系统架构.md) | 5 层 17 系统完整地图 | v1.0 |
| [玩法与平衡性设计](./design/玩法与平衡性设计.md) | Build系统/资源/数值/广告 | v0.5 |
| [组合性随机性系统](./design/组合性随机性系统设计.md) | 元素反应/词缀/怪物装配/变异 | v0.5 |
| [项目约定与版权声明](./design/项目约定与版权声明.md) | 编码规范/版权规避/提灯对比 | v1.0 |
| [文件管理规范](./design/文件管理规范.md) | 文件命名/备份/版本控制 | v1.0 |
| [启动屏设计](./design/风格/启动屏设计.md) | 微信小游戏合规启动屏 | v1.0 |
| [战斗系统规格](./docs/战斗系统规格.md) | 实时自动攻击系统完整规格 | v2.0 |
| [地牢生成系统规格](./docs/地牢生成系统规格.md) | DAG 生成/房间类型/地形模板/种子 | v1.0 |
| [数据埋点方案](./docs/数据埋点方案.md) | 12 个核心事件/微信上报 | v1.0 |
| [本地存储设计](./docs/本地存储设计.md) | 存档结构(500字节)/版本迁移 | v1.0 |
| [合规审查报告](./docs/合规审查报告_微信小游戏.md) | 微信小游戏平台合规检测 | v1.0 |

## 系统规格化进度

| 系统 | 状态 | 优先级 | 文件 |
|------|------|--------|------|
| 战斗系统 | ✅ v2.0 已规格化 | P0 | `docs/战斗系统规格.md` |
| 地牢生成 | ✅ v1.0 已规格化 | P0 | `docs/地牢生成系统规格.md` |
| Build 系统 | ✅ 已适配实时 | P1 | `design/玩法与平衡性设计.md` |
| 资源系统 | ✅ 已确定 | P0 | `design/玩法与平衡性设计.md` |
| 怪物系统 | ✅ 已确定 | P0 | `design/组合性随机性系统设计.md` |
| 元素反应 | ✅ 已确定 | P1 | `design/组合性随机性系统设计.md` |
| 词缀系统 | ✅ 已确定 | P1 | `design/组合性随机性系统设计.md` |
| 长线成长 | ✅ 已确定 | P2 | `design/玩法与平衡性设计.md` |
| 广告系统 | ✅ 已确定 | P3 | `design/玩法与平衡性设计.md` |
| 新手引导 | ✅ 已确定 | P4 | `design/玩法与平衡性设计.md` |
| 数值系统 | ⏳ 框架已定 | P0 | — |
| 事件系统 | ⏳ 概念已定 | P1 | — |
| 房间变异 | ⏳ 概念已定 | P1 | — |
| 经济系统 | ⏳ 框架已定 | P2 | — |
| 种子系统 | ⏳ 概念已定 | P2 | — |
| DDA | ⏳ 概念已定 | P2 | — |
| 跑马灯 | ⏳ 概念已定 | P3 | — |

## 目录结构

```
回到地面/
├── GDD_回到地面.md    # 主设计文档（入口）
├── README.md          # 本文件（项目简介）
├── 目录索引.md         # 文件索引
├── .gitignore         # Git 忽略规则
├── design/            # 设计文档
│   ├── 主体方案确定.md
│   ├── 主体系统架构.md
│   ├── 玩法与平衡性设计.md
│   ├── 组合性随机性系统设计.md
│   ├── 项目约定与版权声明.md
│   ├── 文件管理规范.md
│   └── 风格/
│       └── 启动屏设计.md
├── docs/              # 技术文档
│   ├── 战斗系统规格.md
│   ├── 地牢生成系统规格.md
│   ├── 数据埋点方案.md
│   ├── 本地存储设计.md
│   └── 合规审查报告_微信小游戏.md
├── arts/              # 美术资源（待填充）
├── audio/             # 音频资源（待填充）
├── code/              # 游戏工程（Phase 1 创建）
├── references/        # 参考素材
└── tools/             # 辅助工具
```

## 开发路线图

详细实施计划见 [`docs/实施路线图.md`](./docs/实施路线图.md)

| 阶段 | 内容 | 里程碑 | 周期 |
|------|------|--------|------|
| **Phase 0: 设计** (已完成) | GDD 定稿 + 17 系统全规格化 + 微信小游戏合规 | 全部完成 | ✅ |
| Phase 1: 核心玩法 | 实时战斗 + 网格移动 + 怪物 AI + 地牢探索 + 觉悟战 | 7 个 Milestone | 3 周 |
| Phase 2: 肉鸽系统 | Build 构筑 + 元素反应 + 词缀 + 魂石商店 + 角色 | 4 个 Milestone | 2 周 |
| Phase 3: 内容填充 | 5 层 + 10 怪物 + 5 Boss + 美术 + 音效 | 5 个 Milestone | 2 周 |
| Phase 4: 广告接入 | 激励广告 + 插屏 + Banner + 跑马灯 + 埋点 | 4 个 Milestone | 1 周 |
| Phase 5: 上线准备 | Playtest + 软著 + 自审 + 提审 + 灰度 | 4 个 Milestone | 2 周 |

## 设计原则

1. **走位决策重于手速** —— 实时自动攻击解放操作，走位和技能时机决定胜负
2. **每次旅程都不同** —— 种子驱动的地牢生成/怪物装配/强化选项/房间变异
3. **短平快的节奏** —— 单局 5–15 分钟，战斗 30–90 秒/场
4. **垃圾时间也有价值** —— 跑马灯+觉悟战，让每次死亡都有意义
5. **玩家选择驱动广告** —— 所有广告位均不影响不观看玩家的完整体验

## 版权声明

本游戏所有设计文档、系统框架均为独立原创。游戏机制灵感来源于多款经典 Roguelike 作品，但所有系统实现和数值模型均为原创设计。详见 `design/项目约定与版权声明.md` 中的竞品差异对比。
